...
async def register(
    ...
    if user_data.role not in ["student", "admin", "staff"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Role must be student, admin, or staff."
        )

    account_status = (
        StatusEnum.pending
        if user_data.role == "staff"
        else StatusEnum.approved
    )
    ...
    if user_data.role == "staff":
        if user_data.block is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Staff accounts require a working block."
            )
...
async def login(
    ...
    if user.status == StatusEnum.rejected:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Your account request was rejected. Contact admin."
        )
...
async def request_otp(
    ...
    print(
        f"[MOCK OTP] Mobile: {payload.mobile_number} -> OTP: {otp}"
    )
...
async def verify_otp(
    ...
    if not user or not user.otp_code:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No OTP was requested for this number."
        )
